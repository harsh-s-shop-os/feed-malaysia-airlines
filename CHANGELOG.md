...
**The brand.** Everything is keyed on `malaysiaairlines.com`: Brand Memory, the setup rail, the workspace badge, the deck sidebar and the welcome line. The logo is Malaysia Airlines' own wordmark, taken from the vector in their site code, not redrawn; the workspace mark is the kite cut from it. The kit is brand blue, deep navy, sky and gold, all read off the site's CSS. The intro card copy is untouched; only its three illustrations and wash were regraded from WeWork blue to Malaysia Airlines navy, and the story rings were recoloured to match.
...
**Imagery.** All from the site's own image library. Catalog cards use the destination and cabin photography as shot. Creative cards use the Time for campaign frames, kept whole, never cropped through a headline or a crew member. The three videos are cut from films already on the site. Every WeWork image has been deleted.
...
A new fork for WeWork India, built on the current Cosmix code (the Google Sheet driven build) with the video support from the PUMA fork. It starts with a clean git history of its own.

**The brand.** Everything is keyed on `wework.co.in`: Brand Memory, the setup rail, the workspace badge, the deck sidebar and the welcome line. The logo is WeWork's own wordmark, lifted from the vector on their site, not redrawn. The kit is black, white, WeWork blue and the yellow they use for ratings, all read off the site's CSS. The intro card copy is untouched; only its three illustrations and wash were regraded from Cosmix green to WeWork blue, and the story rings were recoloured to match.
